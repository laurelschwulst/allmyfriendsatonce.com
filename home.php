	...
	<script>
		$(window).load(function (){
			...
			$(document).ready(function () {
			  ...
			  // var regularRock = $('.accordion-row:not(.interstitial) img');
			  
			  // regularRock.mouseover(function(event){
			     //console.log(event.target); 
			     // var title = $(this).closest('.accordion-row').find('.byline').text();
			     // console.log(x);
			     // $('h1').addClass('title-display').text(title);
			  // });
			  // regularRock.mouseout(function(event){
			     // $('h1').removeClass('title-display').text('When all my friends are on at once');
			  // });
			  
			});
			
			function layout() {
			  var totalCols = Math.floor($('.column-container').width() / colWidth);
			  var items = $('.item').remove();
			
			  $('.column').remove();
			
			  for (var i = 0; i < totalCols; ++i) {
			    $('.column-container').append('<div class="column"></div>');
			  };
			
			  var counter = 0;
			  while (counter < items.length) {
			    $('.column').each(function () {
			      $(this).append(items[counter]);
			      ++counter;
			    });
			  };
			};
...
<li><a href="#">Print or Save as PDF</a></li>

<li class="accordion-row">
	<div class="accordion-header credits">
		Credits
	</div>
	<div class="accordion-body">
		Organized by Gene McHugh<br />
		Art by Juan Amaya<br />
		Website by Beautiful Company
	</div>
</li>
